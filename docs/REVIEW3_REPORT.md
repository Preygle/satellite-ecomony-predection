...
**Architecture.** The figure below shows the five layers of the system, each a
folder of the code. Nine architecture diagrams (pipeline data flow, the
typology algorithm, the relative light trend, the growth model, the heat-island
method, the validation framework, the temporal design, the grid) and six
progress charts are generated by `scripts/make_diagrams.py` into
[`diagrams/`](diagrams/); [`DIAGRAMS.md`](DIAGRAMS.md) explains each one.

![System architecture](diagrams/A1_system_architecture.png)
