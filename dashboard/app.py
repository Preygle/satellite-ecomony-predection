...
period = B.get("period", "")
st.markdown(
    f"<div class='ui-caption'>Phase 1 &middot; {summary['aoi']['area_km2']} km² AOI &middot; "
...
if epochs:
    ...
    c1.metric(f"Built-up surface {y1}", f"{b1:,.1f} km²",
              f"{b1 - b0:+,.1f} km² since {y0}")
pop = B.get("population_total", {})
if pop and epochs:
    p0, p1 = pop.get(y0, 0), pop.get(y1, 0)
    c2.metric(f"Population {y1}", f"{p1/1e6:,.2f} M",
...
LAYERS = {
    ...
    "Population": (f"population_{cfg.epoch_current}", "sequential",
                   f"Residential population, {cfg.epoch_current}"),
    "Nightlights": ("nightlights", "sequential", "VIIRS annual mean radiance"),
    "Heat island (SUHI)": ("suhi_intensity", "diverging",
                           "Land surface temperature minus rural reference"),
...
with tab_map:
    if gdf is None or layer_name is None:
        ...
        data = gdf.copy()
        if show_only_urban and "builtup_frac_current" in data.columns:
            data = data[data["builtup_frac_current"] >= 0.20]
...
with tab_ghost:
    ...
    if not G:
        ...
        if zones:
            ...
            cols = [c for c in ["zone_id", "area_km2", "mean_ghost_score", "mean_activity",
                                "mean_residual", "mean_new_builtup_frac", "lat", "lon"]
                    if c in zdf.columns]
            st.dataframe(
                zdf[cols].rename(columns={
                    "zone_id": "Zone", "area_km2": "Area (km²)",
                    "mean_ghost_score": "Ghost score", "mean_activity": "Activity",
                    "mean_residual": "Residual", "mean_new_builtup_frac": "New built-up",
                    "lat": "Lat", "lon": "Lon"}),
...
with tab_env:
    ...
    with ec1:
        ...
        if V:
            ...
            st.markdown(
                "<div class='ui-caption'>Only NDVI decline co-located with built-up "
                "gain is counted as urbanisation-driven. Varanasi sits in intensively "
                "cropped plain, where raw NDVI change mostly tracks the cropping "
                "calendar.</div>", unsafe_allow_html=True)
        else:
    ...
    with ec2:
        ...
        if T and "max_intensity_c" in T:
            st.metric("Peak SUHI intensity", f"+{T['max_intensity_c']:.1f} °C")
            st.metric("Mean urban intensity", f"+{T.get('mean_urban_intensity_c',0):.1f} °C")
            st.metric(f"Hotspot area (≥ +{T.get('hotspot_threshold_c',3)} °C)",
                      f"{T.get('hotspot_area_km2',0):,.1f} km²")
            st.markdown(
                f"<div class='ui-caption'>Rural reference "
                f"{T.get('rural_reference_c','?')} °C, taken from non-built land in the "
                f"same scene with water excluded.</div>", unsafe_allow_html=True)
        else:
